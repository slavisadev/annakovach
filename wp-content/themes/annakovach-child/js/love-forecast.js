jQuery(document).ready(function (e) {
    ...
    let userDetails = JSON.parse(localStorage.getItem('LoveForecastFormData'));
    ...
    function getApiData(api, data, showSpinner) {
        ...
        var request = jQuery.ajax({
            ...
            url: "/wp-content/themes/annakovach-child/api/loveForecastApiCall.php",
            type: "POST",
            dataType: 'json',
            data: (data)
    ...
    function getUserDetails(data) {
        ...
        getApiData('western_horoscope', data, true).then(function (res) {
            getApiData('wheel_chart/tropical', data, false).then(function (img) {
                ...
                getApiData('romantic_forecast_report/tropical', data, false).then(function (forecast) {

                    if(res.status)
                    {
                        let res_cpy = jQuery.extend(true, {}, res);
                        let forecast_cpy = jQuery.extend(true, {}, forecast);
                        res_cpy = JSON.parse(res_cpy.data);
                        forecast_cpy = JSON.parse(forecast_cpy.data);
                        ...
                        jQuery('#userDetails').html(template({
                            "natal": {
                                ...
                                forecast_report : forecast_cpy.romantic_forecast
                            },
                            birth: data,
